...
export const DatewisePage = ({ onBack }) => {
  ...
  if (loading) return <div className="loading-state"><div className="spinner"></div><p>Loading timeline...</p></div>;
  if (error) return <div className="error-state"><p>❌ {error}</p><button onClick={fetchTimeline} className="retry-btn">Retry</button></div>;
    ...
    <div className="datewise-container">
      ...
      <div className="month-title">
        {timelineData.length > 0 
          ? new Date(timelineData[0].Date).toLocaleString('default', { month: 'long' }).toUpperCase() 
          : 'UPDATES'}
      </div>
